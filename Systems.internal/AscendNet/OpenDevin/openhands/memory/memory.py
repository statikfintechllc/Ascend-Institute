...
# ⬇️ Add with your imports — and KEEP THIS FLUSH LEFT!
import chromadb

# ✅ Initialize new-style persistent client
chroma_client = chromadb.PersistentClient(path="./devin_memory")

# ✅ Create or get collection
try:
    chroma_collection = chroma_client.get_collection("devin-memory")
except:
    chroma_collection = chroma_client.create_collection("devin-memory")
...
class Memory:
    ...
    def _find_microagent_knowledge(self, query: str) -> list[MicroagentKnowledge]:
        """Find microagent knowledge based on a query, fallback to ChromaDB vector memory."""
        recalled_content: list[MicroagentKnowledge] = []

        if not query:
            return recalled_content

        # Priority 1: Native OpenDevin microagent triggers
        ...
        # Priority 2: Chroma fallback if nothing matched
    ...
    def store_knowledge_vector(self, name: str, content: str):
        try:
            chroma_collection.add(documents=[content], ids=[name])
            logger.info(f"Stored vector memory for {name}")
        except Exception as e:
            logger.warning(f"Failed to store to ChromaDB: {e}")
