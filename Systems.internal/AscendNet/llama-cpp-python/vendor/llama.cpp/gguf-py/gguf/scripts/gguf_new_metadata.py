...
def copy_with_new_metadata(
    reader: gguf.GGUFReader,
    writer: gguf.GGUFWriter,
    new_metadata: dict[str, MetadataDetails],
    ...
    for field in reader.fields.values():
        ...
        if (
            field.name.startswith(gguf.Keys.Tokenizer.CHAT_TEMPLATE)
            and gguf.Keys.Tokenizer.CHAT_TEMPLATE in new_metadata
        ):
        ...
        if field.name in new_metadata:
            logger.debug(
                f'Modifying {field.name}: "{old_val.value}" -> "{val.value}" {val.description}'
            )
...
def main() -> None:
    tokenizer_metadata = (
        getattr(gguf.Keys.Tokenizer, n)
        for n in gguf.Keys.Tokenizer.__dict__.keys()
        if not n.startswith("_")
    )
    token_names = dict(
        (n.split(".")[-1][: -len("_token_id")], n)
        for n in tokenizer_metadata
    ...
    parser.add_argument(
        "--general-name", type=str, help="The models general.name", metavar='"name"'
    )
    parser.add_argument(
        "--general-description",
        type=str,
        help="The models general.description",
        metavar='"Description ..."',
    )
    parser.add_argument(
        "--chat-template",
        type=str,
        help="Chat template string (or JSON string containing templates)",
        metavar='"{% ... %} ..."',
    )
    parser.add_argument(
        "--chat-template-config",
        type=Path,
        help="Config file containing chat template(s)",
        metavar="tokenizer_config.json",
    )
    parser.add_argument(
        "--pre-tokenizer",
        type=str,
        help="The models tokenizer.ggml.pre",
        metavar='"pre tokenizer"',
    )
    parser.add_argument(
        "--remove-metadata",
        action="append",
        type=str,
        help="Remove metadata (by key name) from output model",
        metavar="general.url",
    )
    parser.add_argument(
        "--special-token",
        action="append",
        type=str,
        help="Special token by value",
        nargs=2,
        metavar=(" | ".join(token_names.keys()), '"<token>"'),
    )
    parser.add_argument(
        "--special-token-by-id",
        action="append",
    ...
    if args.general_name:
        new_metadata[gguf.Keys.General.NAME] = MetadataDetails(
            gguf.GGUFValueType.STRING, args.general_name
        )

    if args.general_description:
        new_metadata[gguf.Keys.General.DESCRIPTION] = MetadataDetails(
            gguf.GGUFValueType.STRING, args.general_description
        )

    if args.chat_template:
        new_metadata[gguf.Keys.Tokenizer.CHAT_TEMPLATE] = MetadataDetails(
            gguf.GGUFValueType.STRING,
            json.loads(args.chat_template)
            if args.chat_template.startswith("[")
    ...
    if args.chat_template_config:
        with open(args.chat_template_config, "r") as fp:
            ...
            if template:
                new_metadata[gguf.Keys.Tokenizer.CHAT_TEMPLATE] = MetadataDetails(
                    gguf.GGUFValueType.STRING, template
                )

    if args.pre_tokenizer:
        new_metadata[gguf.Keys.Tokenizer.PRE] = MetadataDetails(
            gguf.GGUFValueType.STRING, args.pre_tokenizer
        )
    ...
    for name, token in args.special_token or []:
        if name not in token_names:
            ...
            new_metadata[token_names[name]] = MetadataDetails(
                gguf.GGUFValueType.UINT32, ids[0], f"= {token}"
            )

            if len(ids) > 1:
                logger.warning(
                    f'Multiple "{token}" tokens found, choosing ID {ids[0]}, use --special-token-by-id if you want another:'
                )
    ...
    for name, id_string in args.special_token_by_id or []:
        if name not in token_names:
            ...
            if id_int >= 0 and id_int < len(token_list):
                new_metadata[token_names[name]] = MetadataDetails(
                    gguf.GGUFValueType.UINT32, id_int, f"= {token_list[id_int]}"
                )
            else:
