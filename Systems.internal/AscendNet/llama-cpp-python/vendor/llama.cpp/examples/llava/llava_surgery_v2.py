...
def is_vision_tower(weight_name):
    return (
        ...
        or weight_name.startswith("vit.")
        or weight_name.startswith("vision_tower")
    )
...
def is_mm_projector(weight_name):
    return (
        weight_name.startswith("model.mm_projector")
        or weight_name.startswith("vision_proj.")
        or weight_name.startswith("multi_modal_projector")
    )
...
ap.add_argument(
    ...
    "--clean-vision-tower",
    action="store_true",
    help="Remove any vision tower from the model files",
)
args = ap.parse_args()
...
newline_checkpoint_path, projector_checkpoint_path = find_relevant_checkpoints(
    checkpoint_paths, newline_criteria, proj_criteria
)
...
if len(mm_tensors) == 0:
    if last_checkpoint is not None:
        for k, v in last_checkpoint.items():
            print(k)
    print(
        f"Found {len(mm_tensors)} tensors to extract out of {len(last_checkpoint) if last_checkpoint is not None else 0} tensors."
    )
