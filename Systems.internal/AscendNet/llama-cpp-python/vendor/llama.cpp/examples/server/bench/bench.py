...
def main(args_in: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--host", type=str, help="Server listen host", default="0.0.0.0"
    )
    parser.add_argument("--port", type=int, help="Server listen host", default="8080")
    parser.add_argument(
        "--model-path-prefix",
        type=str,
        help="Prefix where to store the model files",
        default="models",
    )
    parser.add_argument(
        "--n-prompts",
    ...
    parser.add_argument(
        "--hf-repo", type=str, help="Hugging Face model repository", required=True
    )
    parser.add_argument(
        "--hf-file", type=str, help="Hugging Face model file", required=True
    )
    parser.add_argument(
        "-ngl",
        "--n-gpu-layers",
        type=int,
        help="layers to the GPU for computation",
        required=True,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--parallel",
        type=int,
        help="Set the number of slots for process requests",
        required=True,
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        help="Set the batch size for prompt processing",
        required=True,
    )
    parser.add_argument(
    ...
    try:
        ...
        with open("results.github.env", "w") as github_env:
            ...
            with open("k6-results.json", "r") as bench_results:
                ...
                iterations = data["root_group"]["checks"]["success completion"][
                    "passes"
                ]
    ...
    if server_process:
        try:
            ...
            print(
                f"server still alive after 500ms, force-killing pid={server_process.pid} ..."
            )
    ...
    if is_server_listening("0.0.0.0", 9090):
        ...
        for metric in metrics:
            ...
            if resp.status_code != 200:
                print(
                    f"bench: unable to extract prometheus metric {metric}: {resp.text}"
                )
            else:
                ...
                timestamps_dt = [
                    str(datetime.fromtimestamp(int(ts))) for ts in timestamps
                ]
                plt.figure(figsize=(16, 10), dpi=80)
                plt.plot(timestamps_dt, metric_values, label=metric)
                plt.xticks(
                    rotation=0, fontsize=14, horizontalalignment="center", alpha=0.7
                )
...
xychart-beta
    ...
    bench_results = {
        ...
        "pp": {
            "p95": round(
                data["metrics"]["llamacpp_prompt_processing_second"]["p(95)"], 2
            ),
            "avg": round(
                data["metrics"]["llamacpp_prompt_processing_second"]["avg"], 2
            ),
            "0": round(mean(prometheus_metrics["prompt_tokens_seconds"]), 2)
            if "prompt_tokens_seconds" in prometheus_metrics
            else 0,
        },
        "tg": {
            ...
            if "predicted_tokens_seconds" in prometheus_metrics
            else 0,
        },
    }
...
def start_benchmark(args):
    ...
    k6_completed = subprocess.run(
        args, shell=True, stdout=sys.stdout, stderr=sys.stderr
    )
...
def start_server_background(args):
    ...
    thread_stdout = threading.Thread(
        target=server_log, args=(server_process.stdout, sys.stdout)
    )
    thread_stdout.start()
    thread_stderr = threading.Thread(
        target=server_log, args=(server_process.stderr, sys.stderr)
    )
    thread_stderr.start()
