...
def test_embedding_openai_library_single():
    ...
    res = client.embeddings.create(
        model="text-embedding-3-small", input="I believe the meaning of life is"
    )
...
def test_embedding_openai_library_base64():
    ...
    res = server.make_request("POST", "/v1/embeddings", data={"input": test_input})
    assert res.status_code == 200
    ...
    res = server.make_request(
        "POST",
        "/v1/embeddings",
        data={"input": test_input, "encoding_format": "base64"},
    )
