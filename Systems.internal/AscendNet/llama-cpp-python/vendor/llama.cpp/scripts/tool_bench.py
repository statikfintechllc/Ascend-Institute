...
def plot(
    ...
    all_cols = [(server_name, test) for server_name in server_names for test in tests]
    for model in models:
        for temp in temps:
    ...
    plt.title(
        f"Tool Call Bench (n = {str(min(total_counts)) if len(total_counts) == 1 else f'{min(total_counts)}-{max(total_counts)}'})\nSuccess Ratios by Server & Test",
        pad=20,
    )
...
def run(
    ...
    model: Annotated[
        Optional[str], typer.Option(help="Name of the model to test (server agnostic)")
    ] = None,
    hf: Annotated[
        Optional[str],
        typer.Option(
            help="GGUF huggingface model repo id (+ optional quant) to test w/ llama-server"
        ),
    ] = None,
    ...
    assert (
        force or append or not output.exists()
