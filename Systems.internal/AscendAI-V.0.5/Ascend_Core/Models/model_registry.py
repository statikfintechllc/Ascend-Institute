...
# ================== CONFIG ==================
MODEL_BASE_PATH = os.path.expanduser(
    "~/AscendAI/AscendNet/AscendAI/Ascend_Core/Models/"
)
...
def validate_models():
    ...
    logging.info(
        f"[VALIDATE] Validation complete. {len(MODEL_REGISTRY)} models remaining."
    )
...
def select_best_model(task="default"):
    valid_models = [
        ...
        if callable(m["call"]) and m["score"] > 60
    ]
    sorted_models = sorted(
        valid_models, key=lambda x: MODEL_REGISTRY[x]["score"], reverse=True
    )
...
def cli():
    ...
    parser.add_argument(
        "--scan", action="store_true", help="Scan and load models from registry path"
    )
    parser.add_argument(
        "--validate", action="store_true", help="Run validate() on all models"
    )
    parser.add_argument("--list", action="store_true", help="List available models")
    parser.add_argument("--prompt", type=str, help="Prompt to run with selected model")
    parser.add_argument(
        "--model", type=str, help="Specify model name to execute prompt"
    )
    parser.add_argument(
        "--fusion", nargs="+", help="Run fusion output across multiple models"
    )
    parser.add_argument(
        "--quantum", action="store_true", help="Check if quantum mode is active"
    )
