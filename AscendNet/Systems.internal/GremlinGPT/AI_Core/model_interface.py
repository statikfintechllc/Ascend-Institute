...
def ask_model(prompt: str, max_tokens=512) -> str:
    ...
    similar = [entry for entry in recent_logs if entry[2] == "error" and prompt[:25].lower() in entry[1].lower()]
...
def auto_remediate_failed_tasks():
    ...
    for log in failed:
        ...
        for root, _, files in os.walk(ROOT_DIR / "tools"):
            for file in files:
                if file.endswith(".py") and file.replace(".py", "").lower() in task_name:
        ...
        if guessed_file:
            ...
            print(f"[REMEDIATION SKIPPED] Could not determine file for task: {task_name}") 